...
from PJ.detect.code.label import shrink
...
def preprocess():
    data_dir = cfg.data_dir     # 数据集放置位置
    origin_image_dir = os.path.join(data_dir, cfg.origin_image_dir_name)    # 原始图片文件夹
    origin_txt_dir = os.path.join(data_dir, cfg.origin_txt_dir_name)    # 原始图片label文件夹
    ...
    draw_gt_quad = cfg.draw_gt_quad     # 是否画ground_truth框
    show_gt_image_dir = os.path.join(data_dir, cfg.show_gt_image_dir_name)      # 画ground_truth框后的图片文件夹
    if not os.path.exists(show_gt_image_dir):
        os.mkdir(show_gt_image_dir)
    show_act_image_dir = os.path.join(cfg.data_dir, cfg.show_act_image_dir_name)    # 实际图片文件夹
    ...
    for o_img_fname, _ in zip(o_img_list, tqdm(range(len(o_img_list)))):
        with Image.open(os.path.join(origin_image_dir, o_img_fname)) as im:
            ...
            im = im.resize((d_wight, d_height), Image.NEAREST).convert('RGB')       # 强制放缩
            show_gt_im = im.copy()
            # draw on the img
