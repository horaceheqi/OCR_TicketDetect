...
def should_merge(region, i, j):
    neighbor = {(i, j - 1)}
    result = region.isdisjoint(neighbor)  #两个集合没有相同元素返回True，有返回False
    return not result
...
def rec_region_merge(region_list, m, S):
    ...
    tmp = []
    for n in S:
        if abs(list(region_list[m])[0][0]-list(region_list[n])[0][0])<2:
            if not region_neighbor(region_list[m]).isdisjoint(region_list[n]) or \
                ...
                # 这里m在上n在下，有必要让n扩展再次判断吗？n的循环没必要走这么长，后面的都是浪费！！！！
                # 第m与n相交
                tmp.append(n)
        else:
            break
